...
<style>
  ...
  /* 차트 레이아웃 개선: 1단 배열로 변경하여 가로폭 확보 */
  .charts { width: 100%; }
  .chart-row { display: grid; grid-template-columns: 1fr; gap: 24px; } 
...
function renderOne(secId){
  ...
  // [개선] 레이아웃: 주가 영역(70%), RSI(15%), MACD(15%)
  const layout1={
    grid:{rows:3,columns:1,pattern:'independent',roworder:'top to bottom'},
    xaxis:{domain:[0,1], rangeslider:{visible:false}, showspikes:true, spikemode:'across'}, 
    yaxis:{domain:[0.35,1.00], title:'주가 (원)', tickformat:',', showspikes:true}, // 메인 차트 영역 확대
    ...
    legend:{orientation:'h', y:1.02, x:0.5, xanchor:'center'}, // 범례 상단 중앙 배치
  ...
  const traces1=[
    // [개선] 캔들스틱 색상: 한국형 (상승:빨강/하락:파랑)
    {type:'candlestick',x:date,open,high,low,close,name:'주가',xaxis:'x',yaxis:'y',
     increasing:{line:{color:'#ef4444'}}, decreasing:{line:{color:'#3b82f6'}} },
    
    // 이동평균선: 색상 구분 명확화
    {type:'scatter',mode:'lines',x:date,y:ma20,name:'MA20',xaxis:'x',yaxis:'y', line:{color:'#f59e0b', width:1.5}},
    {type:'scatter',mode:'lines',x:date,y:ma60,name:'MA60',xaxis:'x',yaxis:'y', line:{color:'#10b981', width:1.5}},
    {type:'scatter',mode:'lines',x:date,y:ma120,name:'MA120',xaxis:'x',yaxis:'y', line:{color:'#6366f1', width:1.5}},
    
    // [개선] 볼린저밴드: 기본적으로 숨김 (legendonly) 처리하여 차트 깔끔하게 시작
    {type:'scatter',mode:'lines',x:date,y:bb.upper,name:'BB상단',xaxis:'x',yaxis:'y', visible:'legendonly', line:{dash:'dot', color:'#9ca3af', width:1}},
    {type:'scatter',mode:'lines',x:date,y:bb.lower,name:'BB하단',xaxis:'x',yaxis:'y', visible:'legendonly', line:{dash:'dot', color:'#9ca3af', width:1}},
  ...
  // [개선] 수급/공매도 차트 레이아웃
  const layout2={
    barmode:'group', // relative -> group 변경으로 매수/매도 비교 용이하게
    yaxis:{title:'순매수 수량', tickformat:',', showgrid:true},
    yaxis2:{title:'공매도 비율(%)', overlaying:'y', side:'right', range:[0, Math.max(...shortBR, ...shortR)*1.2], showgrid:false},
  ...
  const traces2=[
    {type:'bar',x:date,y:inst,name:'기관 순매수', marker:{color:'#fbbf24', opacity:0.8}},
    {type:'bar',x:date,y:foreign,name:'외국인 순매수', marker:{color:'#10b981', opacity:0.8}},
    {type:'scatter',mode:'lines',x:date,y:shortR,name:'공매도비중(%)',yaxis:'y2', line:{color:'#ef4444', width:1, dash:'dot'}},
